...
- `resolveEnemyTurn(state)` — executes the enemy's turn in four steps: (1) ticks active statuses via `tickStatuses` and active auras via `tickAuras` — `tickStatuses` applies Burn and Bleed damage; `tickAuras` calls each active aura's `effectId` via the registry and applies the resulting `SkillOutput` via `applySkillOutput`; both fire before the enemy acts; (2) executes the current intent — `attack` damages the player via `resolveEnemyAttack`, then also applies `getBleedAttackBonus(enemy)` damage back to the enemy via `resolveIncomingDamage` (the wound reopens under exertion), `defend` applies Armor to the enemy via `applyStatuses`, `debuff` is currently a no-op; (3) advances `intentIndex` by 1, wrapping with modulo so the cycle repeats.
