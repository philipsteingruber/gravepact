...
The run map is a flat `MapNode[]` generated by a pure `generateMap(): MapNode[]` function with no Phaser dependency.

```ts
type NodeKind = "combat" | "elite" | "shop" | "rest" | "boss";

interface MapNode {
  id: string;
  kind: NodeKind;
  layer: number;
  connections: string[]; // IDs of reachable nodes in the next layer
}
```
