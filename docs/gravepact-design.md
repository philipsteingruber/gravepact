...
`RunState` holds: `deck`, `hand`, `discardPile`, `relics`, `activeAuras`, `playerHealth`, `playerMaxHealth`, `energyReservation`, and `combatState: CombatState | null`.

> **Open question:** `hand` and `discardPile` are inherently transient combat state — should they live inside `CombatState` rather than `RunState`? The current placement is a holdover from early scaffolding. Decide before the architecture solidifies further. `MetaState` holds: `orbs`, `unlockedCards`, `purchasedUpgrades`, `unlockedClasses`. Only `MetaState` is persisted to localStorage.
